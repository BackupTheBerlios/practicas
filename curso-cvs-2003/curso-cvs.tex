...
\slsect{Contenido}
	...
	\item Método pserver vs. método ssh.
...
%% -------------------------------------------------------------- 
\begin{hslide}
\slsect{Método pserver vs. SSH}
	\begin{itemize}
		\item La autenticación por \textbf{pserver} carece totalmente de
			  seguridad.
		\item Tanto las contraseñas como los ficheros viajan por la
			  red sin cifrar.
		\item Además, un servidor pserver mal configurado puede
			  comprometer la seguridad de toda la máquina.
		\item La ventaja es que los usuarios del servidor pserver no
			  tienen porqué ser usuarios de la máquina.
		\item Con el método \textbf{ssh}, la comunicación con el servidor se realiza de forma
			  segura.
		...
		\item Su uso actualmente sólo está recomendado para cuentas de
			  usuario anónimas.
		...
		\item Muchas de ellas se basan en aplicar parches SSL al
			  servidor CVS.
		\item El problema es que a veces también hace falta parchear
			  los clientes.
		\item Una buena aproximación intermedia es la creación de un
		...
		\item De esta manera, podemos tener cuentas ficticias que se
			  comunicarían con el servidor vía SSH.
		\item Si intentaran acceder por SSH a una shell, estarían
			  dentro de un entorno restringido (la jaula) del que no
			  podrían salir.
		\item Los usuarios normales de la máquina podrían acceder
...
%% -------------------------------------------------------------- 
\begin{hslide}
\slsect{Configuración: método SSH}
	\begin{itemize}
		...
		\end{itemize}
		\item El servidor sólo necesita tener instalado el paquete CVS
			  (es a la vez cliente y servidor).
		\item Cada vez que se ejecute un comando CVS, se deberá
			  introducir la contraseña (son accesos SSH normales y
			  corrientes) a menos que esté configurado el acceso sin
			  contraseña mediante claves públicas.
	\end{itemize}
\end{hslide}  
%% -------------------------------------------------------------- 
%% -------------------------------------------------------------- 
...
\slsect{Una sesión de trabajo típica (I)}
	\begin{itemize}
		\item Nos autenticamos en el servidor (no necesario con método SSH): \\
			  \verb+$ cvs login+ \\
